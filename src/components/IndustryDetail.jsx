...
export function IndustryDetail({ industryId }) {
  ...
  return (
    <>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative", zIndex: 1 }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 32, flexWrap: "wrap" }}>
            ...
            <div style={{ flex: 1, minWidth: 280 }}>
              ...
              <p style={{ fontSize: 17, color: "var(--color-text-secondary)", lineHeight: 1.8, fontWeight: 300, maxWidth: 680 }}>{ind.heroDesc}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Stats */}
      <div style={{ background: "var(--color-surface-elevated)", borderTop: "1px solid var(--color-border)", borderBottom: "1px solid var(--color-border)", padding: "24px 5%" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", gap: 32, justifyContent: "space-around", flexWrap: "wrap" }}>
          {ind.stats.map((s, i) => (
            <div key={i} style={{ textAlign: "center" }}>
              ...
              <div style={{ fontSize: 13, color: "var(--color-text-muted)", marginTop: 2 }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <section style={{ padding: "80px 5%" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div className="rv" style={{ marginBottom: 44 }}>
            <span style={{ display: "inline-block", fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--color-accent)", marginBottom: 14 }}>Why CODEX for {ind.name}</span>
      ...
      <section style={{ background: "var(--color-surface-elevated)", padding: "80px 5%" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div className="rv" style={{ marginBottom: 36 }}>
            ...
            <p style={{ fontSize: 15, color: "var(--color-text-muted)", fontWeight: 300, marginTop: 8 }}>Live, production-tested products purpose-built for this industry.</p>
          </div>
          <div className="rv ig3" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 18 }}>
            {indProducts.map((p) => (
              <Link key={p.id} to={`/product/${p.id}`} className="pcard" style={{ display: "block", background: "var(--color-bg)", border: "1px solid var(--color-border)", borderRadius: 16, padding: "24px 20px", textDecoration: "none" }}>
                <div style={{ fontSize: 36, marginBottom: 10, lineHeight: 1 }}>{p.emoji}</div>
                <div className="sy" style={{ fontSize: 15, fontWeight: 700, marginBottom: 4, color: "var(--color-text)" }}>{p.name}</div>
                <p style={{ fontSize: 12, color: "var(--color-text-subtle)", lineHeight: 1.6 }}>{p.tagline}</p>
                <div style={{ display: "flex", gap: 4, marginTop: 10, flexWrap: "wrap" }}>
                  {p.tags.slice(0, 3).map((t, j) => (
                    <span key={j} style={{ fontSize: 10, padding: "2px 8px", borderRadius: 4, background: "var(--color-glass-bg)", color: "var(--color-accent)", border: "1px solid var(--color-border)" }}>{t}</span>
                  ))}
                </div>
              </Link>
            ))}
      ...
      <section style={{ padding: "80px 5%" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div className="rv" style={{ padding: "24px 28px", background: "var(--color-glass-bg)", border: "1px solid var(--color-border-strong)", borderRadius: 14, borderLeft: "3px solid var(--color-accent)" }}>
            <p style={{ fontSize: 16, color: "var(--color-text-secondary)", lineHeight: 1.8, fontWeight: 300 }}>{ind.audience}</p>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: "var(--color-surface-elevated)", padding: "80px 5%" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div className="rv ig4" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16 }}>
            {relatedInds.map((x) => (
              <Link key={x.id} to={`/industry/${x.id}`} className="pcard" style={{ display: "block", background: "var(--color-bg)", border: "1px solid var(--color-border)", borderRadius: 16, padding: "24px 16px", textAlign: "center", textDecoration: "none" }}>
                <div style={{ fontSize: 36, marginBottom: 8, lineHeight: 1 }}>{x.icon}</div>
                <div className="sy" style={{ fontSize: 13, fontWeight: 700, color: "var(--color-text)" }}>{x.name}</div>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section style={{ background: "linear-gradient(135deg,var(--color-accent-700) 0%,var(--color-accent-900) 50%,var(--color-surface-elevated) 100%)", padding: "80px 5%", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: -100, right: -100, width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle,var(--color-border) 0%,transparent 70%)" }} />
        <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative", zIndex: 1, textAlign: "center" }}>
          ...
          <p style={{ fontSize: 16, color: "var(--color-text-secondary)", fontWeight: 300, maxWidth: 520, margin: "0 auto 28px" }}>Let's discuss your project. Free consultation within 24 hours. Fixed-price contracts with 6-month warranty.</p>
