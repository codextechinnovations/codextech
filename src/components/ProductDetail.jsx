...
export function ProductDetail({ productId }) {
  ...
  return (
    <>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", gap: 32, flexWrap: "wrap" }}>
            ...
            <div style={{ flex: 1, minWidth: 280 }}>
              <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 12, flexWrap: "wrap" }}>
                ...
                <span style={{ fontSize: 11, padding: "4px 10px", background: "var(--color-glass-bg)", color: "var(--color-accent)", borderRadius: 6, fontWeight: 600 }}>● {p.status}</span>
                <span style={{ fontSize: 11, padding: "4px 10px", background: "var(--color-glass-bg)", color: "var(--color-text-subtle)", borderRadius: 6 }}>{p.badge}</span>
              </div>
              ...
              <p style={{ fontSize: 17, color: "var(--color-text-secondary)", lineHeight: 1.8, fontWeight: 300, maxWidth: 680 }}>{e ? e.longDesc : p.desc}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Quick Info Strip */}
      <div style={{ background: "var(--color-surface-elevated)", borderTop: "1px solid var(--color-border)", borderBottom: "1px solid var(--color-border)", padding: "20px 5%" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", gap: 32, flexWrap: "wrap" }}>
          <div>
            <span style={{ fontSize: 12, color: "var(--color-text-muted)" }}>Tech Stack</span>
            <div style={{ display: "flex", gap: 6, marginTop: 6, flexWrap: "wrap" }}>
              {p.tags.map((t, i) => (
                <span key={i} style={{ fontSize: 12, padding: "3px 10px", borderRadius: 5, background: "rgba(14,127,114,0.1)", color: "var(--color-accent)", border: "1px solid rgba(29,207,186,0.15)" }}>{t}</span>
              ))}
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <p style={{ fontSize: 14, fontWeight: 600, marginTop: 4, color: "var(--color-accent)" }}>● {p.status}</p>
          </div>
        </div>
      </div>
        ...
        <section style={{ padding: "80px 5%" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            <div className="rv" style={{ marginBottom: 36 }}>
              ...
              <p style={{ fontSize: 15, color: "var(--color-text-muted)", fontWeight: 300, marginTop: 8 }}>Explore the product in action on these live websites.</p>
            </div>
            <div className="rv" style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 20 }}>
              {e.liveSites.map((site, i) => (
                <a key={i} href={site.url} target="_blank" rel="noopener noreferrer" className="scard" style={{ display: "flex", alignItems: "flex-start", gap: 16, background: "var(--color-surface-elevated)", border: "1px solid var(--color-border)", borderRadius: 16, padding: "28px 24px", textDecoration: "none" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div className="sy" style={{ fontSize: 17, fontWeight: 700, color: "var(--color-accent)", marginBottom: 4 }}>{site.label}</div>
                    <p style={{ fontSize: 14, color: "var(--color-text-muted)", lineHeight: 1.6, marginBottom: 10 }}>{site.desc}</p>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 13, color: "var(--color-accent)", fontWeight: 500 }}>
        ...
        <section style={{ padding: "80px 5%" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            <div className="rv" style={{ marginBottom: 44 }}>
              <span style={{ display: "inline-block", fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--color-accent)", marginBottom: 14 }}>Why Choose This Product</span>
            ...
            <div className="rv g2" style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 20 }}>
              {e.benefits.map((b, i) => (
                <div key={i} className="scard" style={{ background: "var(--color-surface-elevated)", border: "1px solid var(--color-border)", borderRadius: 16, padding: "28px 24px", display: "flex", gap: 16 }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 14, color: "var(--color-text-muted)", lineHeight: 1.7 }}>{b.desc}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* All Features */}
      <section style={{ background: "var(--color-surface-elevated)", padding: "80px 5%" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div className="rv" style={{ marginBottom: 44 }}>
            ...
            <p style={{ fontSize: 16, color: "var(--color-text-muted)", fontWeight: 300 }}>Every tool and capability you need to run your operations efficiently.</p>
          </div>
          <div className="rv g2" style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 14 }}>
            {p.features.map((f, i) => (
              <div key={i} className="icard" style={{ display: "flex", alignItems: "center", gap: 14, padding: "18px 20px", background: "var(--color-bg)", border: "1px solid var(--color-border)", borderRadius: 12 }}>
                ...
                <span style={{ fontSize: 15, color: "var(--color-text-secondary)" }}>{f}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <section style={{ padding: "80px 5%" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            ...
            <div className="rv ig4" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>
              {e.modules.map((m, i) => (
                <div key={i} className="tcard" style={{ background: "var(--color-surface-elevated)", border: "1px solid var(--color-border)", borderRadius: 14, padding: "28px 22px", textAlign: "center" }}>
          ...
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            ...
            <div className="rv" style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
              {e.integrations.map((int, i) => (
                <span key={i} className="tpill" style={{ padding: "10px 20px", borderRadius: 8, border: "1px solid var(--color-border)", background: "var(--color-glass-bg)", fontSize: 14, fontWeight: 500, color: "var(--color-text-secondary)", display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--color-accent-700)", flexShrink: 0 }} />{int}
                </span>
              ))}
            </div>
          </div>
      ...
      {e && e.audience && (
        <section style={{ padding: "80px 5%" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            ...
            <div className="rv" style={{ padding: "24px 28px", background: "rgba(14,127,114,0.07)", border: "1px solid rgba(29,207,186,0.18)", borderRadius: 14, borderLeft: "3px solid var(--color-accent)" }}>
              <p style={{ fontSize: 16, color: "var(--color-text-secondary)", lineHeight: 1.8, fontWeight: 300 }}>{e.audience}</p>
            </div>
          </div>
        </section>
      )}
        ...
        <section style={{ background: "var(--color-surface-elevated)", padding: "80px 5%" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            ...
            <div className="rv ig" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 18 }}>
              {related.map((r) => (
                <Link key={r.id} to={`/product/${r.id}`} className="pcard" style={{ display: "block", background: "var(--color-bg)", border: "1px solid var(--color-border)", borderRadius: 16, padding: "24px 20px" }}>
                  ...
                  <p style={{ fontSize: 12, color: "var(--color-text-subtle)", lineHeight: 1.6 }}>{r.tagline}</p>
                </Link>
              ))}
            </div>
          </div>
      ...
      {/* CTA */}
      <section style={{ background: "linear-gradient(135deg,var(--color-accent-700) 0%,var(--color-accent-900) 50%,var(--color-surface-elevated) 100%)", padding: "80px 5%", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative", zIndex: 1, textAlign: "center" }}>
          ...
          <p style={{ fontSize: 16, color: "var(--color-text-secondary)", fontWeight: 300, maxWidth: 520, margin: "0 auto 28px" }}>Let's discuss how we can tailor this solution for your specific business needs. Free consultation within 24 hours.</p>
          <Link to="/?section=contact" className="bw" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#fff", color: "var(--color-accent-700)", padding: "15px 32px", borderRadius: 9, fontSize: 15, fontWeight: 700, boxShadow: "0 4px 20px rgba(0,0,0,0.2)" }}>
